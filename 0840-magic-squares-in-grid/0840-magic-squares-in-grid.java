class Solution {
		...
		for(int i=0; i<3; i++){
			...
			for(int j=0; j<3; j++){
				...
				if(grid[x+i][y+j] < 1 ||grid[x+i][y+j] > 9 
                             || count[grid[x+i][y+j]] != 0)	return false;
				count[grid[x+i][y+j]] = 1;

			}
			if(sum1 != sum || sum != sum2 || sum1 != sum2)	return false;
		}
		sum1 = grid[x][y] + grid[x+1][y+1] + grid[x+2][y+2];
		sum2 = grid[x][y+2] + grid[x+1][y+1] + grid[x+2][y];
        
		if(sum1 != sum2 || sum1 != sum) return false;
		return true;
	}
    ...
    public int numMagicSquaresInside(int[][] grid) {
		...
		for(int i=0; i<n-2; i++)
			for(int j=0; j<m-2; j++) if(sum(i, j, grid)) count++;
