class Solution {
    public String reorganizeString(String s) {
        ...
        while(maxHeap.size() > 1){
            ...
            if(frequencyMap.get(firstMostCountChar) != 0) maxHeap.add(firstMostCountChar);
            if(frequencyMap.get(secondMostCountChar) != 0) maxHeap.add(secondMostCountChar);
        }
        
        if(!maxHeap.isEmpty()){
            char c = maxHeap.remove();
            if(frequencyMap.get(c) > 1) return "";
            builder.append(c);
        }
